...
setup_backup_job() {
    ...
    read -p "Enter backup interval in minutes (e.g., 120 for every 2 hours): " INTERVAL
...
#!/bin/bash
# This script is auto-generated by the mersyar manager. Do not edit manually.
...
echo "Dumping database from container '\$DB_CONTAINER'..."
# Read DB password from .env file securely
...
EOF
    ...
    cron_job="*/\$INTERVAL * * * * bash ${PROJECT_DIR}/backup_script.sh '$BACKUP_BOT_TOKEN' '$BACKUP_CHAT_ID' > /dev/null 2>&1 # MERSYAR_BACKUP_JOB"
    
    # Remove any old backup job and add the new one to prevent duplicates
    (crontab -l 2>/dev/null | grep -v "# MERSYAR_BACKUP_JOB" ; echo "\$cron_job") | crontab -
    
    success "Backup job successfully scheduled!"
    echo "A backup will be sent to your channel every \$INTERVAL minutes."
}
