# modules/marzban/actions/api.py
# (کد کامل و جایگزین)
...
from .data_manager import normalize_username, load_marzban_credentials

LOGGER = logging.getLogger(__name__)
_client = httpx.AsyncClient(timeout=20.0, http2=True)

async def get_marzban_token() -> Optional[str]:
    credentials = await load_marzban_credentials()
    base_url = credentials.get("base_url")
    username = credentials.get("username")
    password = credentials.get("password")

    if not all([base_url, username, password]):
        LOGGER.warning("Marzban API call failed: Credentials are not set in the bot.")
        return None
...
# تمام توابع دیگر نیز باید ابتدا توکن را دریافت کنند
# پس ساختارشان شبیه به هم خواهد بود
...
# بازنویسی توابع API با استفاده از تابع کمکی
...
async def get_user_data(username: str) -> Optional[Dict[str, Any]]:
    ...
    if response and "error" in response:
        LOGGER.error(f"API error while fetching user '{username}': {response['error']}")
        return None
...
async def modify_user_api(username: str, settings_to_change: dict) -> Tuple[bool, str]:
    ...
    if 'proxies' in current_data and isinstance(current_data['proxies'], dict):
        cleaned_proxies = {
            proto: settings 
            for proto, settings in current_data['proxies'].items() 
            if settings 
        }
        current_data['proxies'] = cleaned_proxies

    # --- This is the key fix ---
    # When renewing a user (especially an expired one), their status might be
    # 'expired' or 'limited', which are not valid inputs for the API.
    # We must explicitly set the status to 'active' for the modification to succeed.
    ...
    error_msg = response.get("error", "Unknown error") if response else "Network error"
    return False, error_msg
async def delete_user_api(username: str) -> Tuple[bool, str]:
    # Use the original username directly without normalization
