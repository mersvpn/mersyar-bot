# FILE: modules/marzban/actions/display.py (FINAL FIX for variable name collision)
...
# No global import of translator to prevent any potential circular issues
...
def get_user_display_info(user: dict) -> tuple[str, str, bool, str, str]:
    ...
    prefix = _("marzban_display.status_active")
    is_online = False
    days_left_str = _("marzban_display.infinite")
    data_left_str = _("marzban_display.infinite")
    ...
    if online_at:
        try:
            ...
            if (datetime.datetime.now(datetime.timezone.utc) - online_at_dt).total_seconds() < 180:
                is_online = True
                prefix = _("marzban_display.status_online")
        except (ValueError, TypeError): pass

    if status != 'active' or (expire_timestamp and datetime.datetime.fromtimestamp(expire_timestamp) < datetime.datetime.now()):
        prefix = _("marzban_display.status_inactive")
        days_left_str = _("marzban_display.expired")
    else:
        is_warning = False
        if expire_timestamp:
            ...
            days_left_str = _("marzban_display.days_left", days=days_left_val)
            if 0 < days_left_val <= 3:
                is_warning = True
        
        if data_limit > 0:
            data_left_gb = (data_limit - used_traffic) / GB_IN_BYTES
            data_left_str = _("marzban_display.data_left_gb", gb=data_left_gb)
            if data_left_gb < 1:
                is_warning = True
        
        if is_warning and not is_online:
            prefix = _("marzban_display.status_warning")
...
def build_users_keyboard(users: list, current_page: int, total_pages: int, list_type: str) -> InlineKeyboardMarkup:
    ...
    max_len = 0
    if display_data:
        max_len = max(len(prefix + " " + username) for prefix, username, _unused1, _unused2, _unused3 in display_data)
    ...
    for i, user_data in enumerate(display_data):
        ...
        if is_online:
            details_part = f"{_('marzban_display.online_label')}  | 📶 {data_left}"
        else:
    ...
    nav_row = []
    if current_page > 1:
        nav_row.append(InlineKeyboardButton(_("marzban_display.pagination_prev"), callback_data=f"show_users_page_{list_type}_{current_page - 1}"))
    if total_pages > 1:
        nav_row.append(InlineKeyboardButton(_("marzban_display.pagination_page_info", current=current_page, total=total_pages), callback_data="noop"))
    if current_page < total_pages:
        nav_row.append(InlineKeyboardButton(_("marzban_display.pagination_next"), callback_data=f"show_users_page_{list_type}_{current_page + 1}"))
    if nav_row:
        keyboard.append(nav_row)
        
    keyboard.append([InlineKeyboardButton(_("marzban_display.pagination_close"), callback_data="close_pagination")])
    return InlineKeyboardMarkup(keyboard)

@admin_only
async def show_user_management_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    from shared.translator import _
    await update.message.reply_text(_("marzban_display.user_management_section"), reply_markup=get_user_management_keyboard())

async def _list_users_base(update: Update, context: ContextTypes.DEFAULT_TYPE, list_type: str, page: int = 1):
    from shared.translator import _
    is_callback = update.callback_query is not None
    message = update.callback_query.message if is_callback else await update.message.reply_text(_("marzban_display.loading"))
    ...
    if is_callback:
        ...
        await message.edit_text(_("marzban_display.fetching_users"))
    
    target_users = []
    
    try:
        if list_type == 'search':
            title_text = _("marzban_display.search_results_title")
            not_found_text = _("marzban_display.no_search_results")
            target_users = context.user_data.get('last_search_results', [])
        else:
            title_text = _("marzban_display.warning_list_title") if list_type == 'warning' else _("marzban_display.all_users_list_title")
            not_found_text = _("marzban_display.no_warning_users") if list_type == 'warning' else _("marzban_display.no_users_in_panel")
            
            all_users = await get_all_users()
            if all_users is None:
                await message.edit_text(_("marzban_display.panel_connection_error")); return
            
            if list_type == 'warning':
                warning_users = []
                warning_status = _("marzban_display.status_warning")
                inactive_status = _("marzban_display.status_inactive")
                for u in all_users:
                    # ✨✨✨ KEY FIX HERE ✨✨✨
                    # Changed `_` to `_unused` to avoid name collision with the translator function.
                    prefix, _unused1, is_online, _unused2, _unused3 = get_user_display_info(u)
        ...
        safe_title = escape_markdown(_("marzban_display.page_title", title=title_text, page=page), version=2)
        ...
        # Use a fresh import of `_` here just in case it was overwritten before the error
        from shared.translator import _
        await message.edit_text(_("marzban_display.list_display_error"))
...
# ... The rest of the functions (show_user_details_panel, show_user_details, etc.) remain the same as the last version with local imports ...
async def show_user_details_panel(context: ContextTypes.DEFAULT_TYPE, chat_id: int, username: str, list_type: str, page_number: int, success_message: str = None, message_id: int = None) -> None:
    ...
    if not user_info:
        if message_id:
            try:
                await context.bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=_("marzban_display.user_not_found"))
            except Exception:
                await context.bot.send_message(chat_id=chat_id, text=_("marzban_display.user_not_found"))
        return

    online_status = _("marzban_display.offline")
    if user_info.get('online_at'):
        try:
            online_at_dt = datetime.datetime.fromisoformat(user_info['online_at'].replace("Z", "+00:00"))
            if (datetime.datetime.now(datetime.timezone.utc) - online_at_dt).total_seconds() < 180:
                online_status = _("marzban_display.online")
        except (ValueError, TypeError): pass
        
    used_gb = (user_info.get('used_traffic') or 0) / GB_IN_BYTES
    limit_gb = (user_info.get('data_limit') or 0) / GB_IN_BYTES
    usage_str = f"{used_gb:.2f} GB / " + (f"{limit_gb:.0f} GB" if limit_gb > 0 else _("marzban_display.unlimited"))
    
    expire_str = _("marzban_display.unlimited")
    if user_info.get('expire'):
        ...
        if expire_dt > datetime.datetime.now():
            ...
            expire_str = f"{jalali_date.strftime('%Y/%m/%d')} (" + _("marzban_display.days_remaining", days=days_left) + ")"
        else:
            expire_str = _("marzban_display.expired")
            
    message_text = ""
    if success_message:
        message_text += f"{success_message}\n{'-'*20}\n"
    
    message_text += _("marzban_display.user_details_title", username=username)
    message_text += f"{_('marzban_display.user_status_label')} {online_status}\n"
    message_text += f"{_('marzban_display.user_usage_label')} {usage_str}\n"
    message_text += f"{_('marzban_display.user_expiry_label')} `{expire_str}`"
    
    back_button_callback = f"list_subs_page_{page_number}" if list_type == 'subs' else f"show_users_page_{list_type}_{page_number}"
    back_button_text = _("marzban_display.back_to_subs_list") if list_type == 'subs' else _("marzban_display.back_to_users_list")
    
    keyboard_rows = [
        [InlineKeyboardButton(_("marzban_display.button_smart_renew"), callback_data=f"renew_{username}"), InlineKeyboardButton(_("marzban_display.button_send_invoice"), callback_data=f"send_invoice_{username}")],
        [InlineKeyboardButton(_("marzban_display.button_add_data"), callback_data=f"add_data_{username}"), InlineKeyboardButton(_("marzban_display.button_add_days"), callback_data=f"add_days_{username}")],
        [InlineKeyboardButton(_("marzban_display.button_reset_traffic"), callback_data=f"reset_traffic_{username}"), InlineKeyboardButton(_("marzban_display.button_subscription_info"), callback_data=f"note_{username}")],
        [InlineKeyboardButton(_("marzban_display.button_subscription_link"), callback_data=f"sub_link_{username}"), InlineKeyboardButton(_("marzban_display.button_delete_user"), callback_data=f"delete_{username}")],
...
async def show_user_details(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await query.edit_message_text(_("marzban_display.list_display_error"))
        return
    context.user_data['current_list_type'] = list_type
    context.user_data['current_page'] = page_number
    loading_message = None
    loading_text = _("marzban_display.getting_details_for", username=f"`{username}`")
...
async def close_pagination_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_message(
        ...
        text=_("marzban_display.back_to_user_management"),
...
async def handle_deep_link_details(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(_("marzban_display.invalid_link"))
        await show_main_menu_action(update, context)
        return
    loading_msg = await update.message.reply_text(_("marzban_display.getting_details_for", username=f"`{username}`"), parse_mode=ParseMode.MARKDOWN)
    await show_user_details_panel(
    ...
    if inbounds:
        for protocol, link_list in inbounds.items():
            if link_list:
                ...
                links_text += "\n"
    if not links_text:
        return _("marzban_display.sub_link_not_found")
...
async def send_subscription_qr_code_and_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await query.edit_message_text(_("marzban_display.internal_error_username_not_found"))
        return
    user_data = await get_user_data(username)
    subscription_url = user_data.get('subscription_url')
    if not subscription_url:
        await query.edit_message_text(text=_("marzban_display.link_not_found_for_user", username=f"`{username}`"), parse_mode=ParseMode.MARKDOWN)
        return
    ...
    bio.seek(0)
    caption = _("marzban_display.qr_caption", username=f"`{username}`", url=f"`{subscription_url}`")
    ...
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton(_("marzban_display.back_to_user_details"), callback_data=back_button_callback)
    ]])
    await query.message.delete()
