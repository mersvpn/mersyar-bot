...
async def approve_payment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        invoice_id_str = query.data.split('_')[-1]
        if not invoice_id_str.isdigit():
            raise ValueError("Invoice ID is not a number")
...
async def reject_payment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await update_invoice_status(invoice['id'], 'rejected'):
        await query.edit_message_caption(caption=f"{query.message.caption}\n\n{_('financials_payment.error_updating_invoice_db')}")
        return
        
    LOGGER.info(f"Admin {admin_user.id} rejected payment for invoice #{invoice['id']}.")
    
    try:
        await context.bot.send_message(invoice['user_id'], _("financials_payment.payment_rejected_customer_message", id=invoice['id']))
    except Exception as e:
